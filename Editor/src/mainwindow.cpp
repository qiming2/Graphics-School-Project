...
void MainWindow::NonInteractiveRenderHelper(QString filename1, QString filename2) {
    Scene* new_scene = scene_manager_.LoadScene(filename1.toStdString());
    if (new_scene != nullptr) {
        current_file_path_ = filename1.toStdString();
        GrayoutConsole();
        SetScene(*new_scene);
    }

    AnimationSettings settings(animator_.GetFPS(), 0, filename2.toStdString(), true);
    render_window_.exec(*scene_, settings);
}
